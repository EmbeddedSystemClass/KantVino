...
namespace KantVinoV2
{
    class ComPort
    {
        ...
        private readonly int[] auchCRCLo = new int[256]
        {
        ...
        public int CRC16_Check(ref byte[] buf, int begin, int end, bool isWrite = false)
        {
            ...
            int crc_lo = 0xFF; // low byte of CRC initialized

            for (; begin < end; begin++)
            {
                j = crc_hi ^ buf[begin]; // will index into CRC lookup table
                crc_hi = crc_lo ^ auchCRCHi[j]; // calculate the CRC
                crc_lo = auchCRCLo[j];
            }
            if (isWrite)
            {
                buf[begin++] = (byte)crc_hi;
                buf[begin++] = (byte)crc_lo;
            }
